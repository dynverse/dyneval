score_map_node <- function(permutation, net, net_ref) {
...
#' @importFrom GA ga
calculate_node_edit_score <- function(
  net1,
  net2,
  ...
  optimize_node_edit_score(net1, net2, nodes1, nodes2, normalize_weights)$best_score
}

optimize_node_edit_score <- function(
  net1,
  net2,
  nodes1=unique(c(net1$from, net1$to)),
  nodes2=unique(c(net2$from, net2$to)),
  ...
  if (nrow(net) < 10) {
    ...
    scores <- permutations %>% apply(1, score_map_node, net=net, net_ref=net_ref)

    list(best_permutation = permutations[which.max(scores), , drop=F], best_score = max(scores))
  } else {
    results <- GA::ga("permutation", score_map_node, net=net, net_ref=net_ref, min=rep(1, length(nodes1)), max=rep(length(nodes1), length(nodes1)), maxiter=100, monitor=FALSE, popSize=500, maxFitness = 1)

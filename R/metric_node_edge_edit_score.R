...
calculate_node_edge_edit_score <- function(
  ...
  if(map_size <= 11) {
    ...
    scores <- map_dbl(combinations, score_map_node_edge, map_grid, net1, net2, nodes1, nodes2)
    score <- max(scores)
  } else {
    permutation2map <- function(perm) {
      (1:nrow(map_grid)) %in% ((0:(length(nodes1)-1)) * length(nodes1) + perm)
    }
    results <- optimize_node_edit_score(net1, net2)
    perm <- results$best_permutation
    initial <- apply(perm, 1, permutation2map) %>% t

    popSize <- 50
    if(nrow(initial) > popSize) initial <- initial[1:popSize, ]

    results <- GA::ga("binary", score_map_node_edge, map_grid, net1, net2, nodes1, nodes2, nBits = map_size, maxiter=50, monitor=FALSE, popSize=popSize, run=5, suggestions=initial, maxFitness=1)
    score <- results@fitnessValue
  }
  score
}

score_map_node_edge <- function(map, map_grid, net1, net2, nodes1, nodes2) {

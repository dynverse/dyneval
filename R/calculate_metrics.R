...
#' @importFrom dynwrap is_wrapper_with_waypoint_cells compute_geodesic_distances
#'
#' @export
calculate_metrics <- function(
  ...
  if (("correlation" %in% metrics)) {
    ...
    if (!is.null(model)) {
      ...
      time0 <- Sys.time()
      dataset$geodesic_dist <- dynwrap::compute_geodesic_distances(dataset, waypoints)
      model$geodesic_dist <- dynwrap::compute_geodesic_distances(model, waypoints)
      time1 <- Sys.time()

...
waterfall <- function(counts, group.index, k, verbose=F) {
  ...
  pseudotime.y <- update.updown / (max(pseudotime) - min(pseudotime))
  ...
  path <- data.frame(Comp1=pseudotime.flow, Comp2=rep(0,length(pseudotime.flow)), row.names=NULL)

  space <- data.frame(Comp1=pseudotime, Comp2=pseudotime.y)
  rownames(space) <- rownames(counts)

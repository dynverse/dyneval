...
# ggplot(task$milestone_percentages) + geom_point(aes(MDP, CDP, colour = PreDC)) + scale_colour_distiller(palette = "RdBu")
# pheatmap::pheatmap(t(as.matrix(task$milestone_percentages[,-1])))

output <- dyneval:::trainLearner.ti.scorpius(
  task, .subset = NULL, num_dimensions = 3, num_clusters = 4)

output$milestone_network
output$milestone_percentages

# todo:
#  - compare task$milestone_network and task$milestone_percentages
#  - to output$milestone_network and output$milestone_percentages
gold <- as.matrix(task$milestone_percentages[,-1])
pred <- as.matrix(output$milestone_percentages[,-1])
...
ggplot() +
  geom_point(aes(output$milestone_percentages$A, task$milestone_percentages$MDP, colour = "MDP")) +
  geom_point(aes(output$milestone_percentages$A, task$milestone_percentages$CDP, colour = "CDP")) +
  geom_point(aes(output$milestone_percentages$A, task$milestone_percentages$PreDC, colour = "PreDC"))
...
vectt<-data.frame(from=firs$name,to=las$name,length=1) # filling milestone_network
...
milestone_network<-vectt


# computing only branch probabilities (2 probs per cell)?
special_names<-unique(c(as.character(vectt[,1]),as.character(vectt[,2])))
milestone_percentages<-matrix(rep(0,length(special_names)*length(V(gr))),nrow=length(V(gr)))
colnames(milestone_percentages)<-special_names
rownames(milestone_percentages)<-V(gr)$name

for ( i in 1:length(nodelist)){
  for (j in 1:length(nodelist[[i]])){
    row2fil<-which(rownames(milestone_percentages)==nodelist[[i]][j]$name)
    col1<-which(colnames(milestone_percentages)==as.character(vectt[i,1]))
    col2<-which(colnames(milestone_percentages)==as.character(vectt[i,2]))
    dist_node<-c(distances(gr,v=nodelist[[i]][1],to=nodelist[[i]][j]),
           distances(gr,v=nodelist[[i]][j],to=nodelist[[i]][length(nodelist[[i]])]))
    milestone_percentages[row2fil,col1]<-1-(dist_node[1]/sum(dist_node))
    milestone_percentages[row2fil,col2]<-1-(dist_node[2]/sum(dist_node))
  }
}
